import { Container, FormControl, InputLabel, Select, FormControlLabel, RadioGroup, Radio, Typography, MenuItem } from '@material-ui/core';
import ImageHeader from './ImageHeader';
import Button from '../Button';
import TextField from '../TextField';
import Link from '../Link'
import React, { useState, useEffect } from 'react';
import { useStyles } from '../../Styles/FormsStyles/RegisterRegularForm';
    ...
    return (
        <form onSubmit={sendOTPCode} >
            ...
            <Container className={classes.container}>
                <ImageHeader pictureClass="RegisterImageHeader" headerText={data.Form_Title} />
                <div className="Spacer"></div>
                <div id="recaptcha"></div>
                <Container className={classes.flexGroup}>
                    <TextField label={data.First_Name} required={true}
                        onChange={(e) => { changeRegistrationFeild(e, "firstName") }} />

                    <TextField label={data.Last_Name} required={true}
                        onChange={(e) => { changeRegistrationFeild(e, "lastName") }}
                    />
                </Container>

                <Container>
                    <TextField label={data.Phone_Number} required={true}
                        onChange={(e) => { changeRegistrationFeild(e, "phoneNumber") }}
                    />
                </Container>

                <Container className={classes.flexGroup}>
                    <TextField label={data.Password} type="password" required={true}
                        onChange={(e) => { changeRegistrationFeild(e, "password") }} />
                    <TextField label={data.Confirm_Password} type="password" required={true}
                        onChange={(e) => { changeRegistrationFeild(e, "confirmPassword") }} />
                </Container>
                        ...
                        <FormControl className={classes.flexGroup}>
                            ...
                            <RadioGroup row defaultValue={"Male"} onChange={(e) => { changeRegistrationFeild(e, "gender") }}>
                                <FormControlLabel
                                    ...
                                    value={gender.genderlist[0].value} label={gender.genderlist[0].type} />
                                <FormControlLabel
                                    control={<Radio color="secondary" className={classes.radio} />}
                                    value={gender.genderlist[1].value} label={gender.genderlist[1].type}
                                />
                            </RadioGroup>
                        </FormControl>
                    </Container>
                ...
                <Container className={classes.centerItems}>
                    <Button color="secondary" size="large" halfWidth variant="contained">{data.Register_Btn}</Button>
                </Container>
                <div className="Spacer"></div>
                <Link to="/register/lawyer">{data.LawyerQuestionMark}</Link>
                {showVerficationCode &&
                    <Container className={classes.flexGroup} >
